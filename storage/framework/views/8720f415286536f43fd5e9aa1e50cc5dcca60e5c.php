<?php $__env->startSection('title', 'Add Food Category'); ?>
<?php $__env->startSection('content'); ?>

<div class="row">
    <div class="col-md-8">
        <div class="card">

            <div class="card-header card-header-icon" data-background-color="purple">
                    <i class="material-icons">add</i>
                </div>
                <div class="card-content">
                    <h4 class="card-title">Add Food Category</h4>

                    <?php echo Form::open(['route' => 'admin.create.category', 'method' => 'POST', 'id' => 'RegisterValidation']); ?>


                    <div class="row">
                        <div class=" col-md-12">
                            <div class="form-group label-floating">                                
                                <?php echo e(Form::label('cat_id', 'Category Id', ['class' => 'control-label container-label'])); ?>

                                <?php echo e(Form::text('cat_id', null, ['class' => 'form-control border-input'])); ?>

                            </div>
                            <div class="form-group label-floating">                                
                                <?php echo e(Form::label('food_cat_name', 'Food Category Name', ['class' => 'control-label container-label'])); ?>

                                <?php echo e(Form::text('food_cat_name', null, ['class' => 'form-control border-input', 'required' => ''])); ?>

                            </div>
                            <div class="form-group label-floating">                                
                                <?php echo e(Form::label('details', 'Details', ['class' => 'control-label container-label'])); ?>

                                <?php echo e(Form::textarea('details', null, ['class' => 'form-control border-input', 'rows' => 5])); ?>

                            </div>
                            <button type="submit" class="btn btn-primary pull-right"><i class="material-icons">add</i> Add</button>
                        </div>
                    </div>

                    <?php echo Form::close(); ?>

                    
                </div>
            </div>
        </div>
    </div>

<?php $__env->stopSection(); ?>


<?php echo $__env->make('admin', array_except(get_defined_vars(), array('__data', '__path')))->render(); ?>